...
export const DefectoLecturaOCRSchema = z.enum([
...
export type DefectoLecturaOCR = z.infer<typeof DefectoLecturaOCRSchema>;

export const ReporteOCRSchema = z.object({
  // ISO 8601 (nunca Date).
  ...
  consumo: z.number().optional(), // Acumulado reportado (= lectura del odómetro).
  consumoCorregido: z.number().optional(), // Acumulado +- consumoInicial cargado en la plataforma.
  ...
  intervaloReporteSegundos: z.number().optional(),

  // Fuente 1: lectura del propio dispositivo (OCR on-device).
  ocrReadingDevice: z.number().optional(),
  ...
  estadoOcr: z
    .enum(["pendiente", "revision", "auto", "confirmado", "corregido"])
    .optional(),
  ...
  defectosLectura: z.array(DefectoLecturaOCRSchema).optional(),
});
export type IReporteOCR = z.infer<typeof ReporteOCRSchema>;
